...
<%
    ...
    String sessionRole = (String) session.getAttribute("role");  // 역할 받아오기
    System.out.println("Debug: sessionId = " + sessionId + ", role = " + sessionRole);
%>
    ...
    <ul class="nav">
      ...
      <c:choose>
        ...
        <c:otherwise>
          ...
          <li class="nav-item">
            <a class="nav-link text-danger" href="<c:url value='/cart.jsp'/>">장바구니</a>
          </li>
          <li class="nav-item">
            <a class="nav-link text-danger" href="<c:url value='/member/logout.jsp'/>">로그아웃</a>
          </li>
          <li class="nav-item">
            <a class="nav-link text-primary" href="<c:url value='/updateMember.jsp'/>">회원정보 수정</a>
          </li>
